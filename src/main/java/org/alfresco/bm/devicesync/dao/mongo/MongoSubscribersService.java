 ...
 *
 */
public class MongoSubscribersService implements SubscribersService,
        InitializingBean
{
    ...
    private void checkIndexes()
    {
        ...
        DBObject uidxSubscriberId = BasicDBObjectBuilder.start(
                FIELD_SUBSCRIBER_ID, 1).get();
        DBObject optSubscriberId = BasicDBObjectBuilder
                .start("name", "uidxSubscriberId").add("unique", Boolean.TRUE)
                .add("sparse", Boolean.TRUE).get();
        collection.createIndex(uidxSubscriberId, optSubscriberId);

        DBObject idxUserName = BasicDBObjectBuilder.start(FIELD_USERNAME, 1)
                .add(FIELD_STATE, 2).get();
        DBObject optUserName = BasicDBObjectBuilder
                .start("name", "uidxUserName").add("unique", Boolean.FALSE)
                .get();
            ...
            String syncServiceURI, DataCreationState state)
    {
        SubscriberData subscriberData = new SubscriberData(username,
                subscriberId, syncServiceURI, state);
        DBObject insert = subscriberData.toDBObject();
        collection.insert(insert);
    }

    @Override
    public void updateSubscriber(ObjectId objectId, String subscriberId,
    ...
    {
        DBObject query = QueryBuilder.start(FIELD_USERNAME).is(username).get();
    ...
    @Override
    public List<SubscriberData> getSubscribers(DataCreationState state,
            int skip, int count)
    {
        ...
        for (DBObject dbObject : cursor)
        {
            ...
            // String subscriberId = (String)dbObject.get(FIELD_SUBSCRIBER_ID);
            // SubscriberData subcriber = new SubscriberData(objectId, username,
            // subscriberId);
    ...
    {
        QueryBuilder rangeBuilder = QueryBuilder.start(FIELD_STATE).is(
        ...
        QueryBuilder queryBuilder = QueryBuilder.start(FIELD_STATE).is(
                DataCreationState.Created.toString());
        if (username != null)
        {
            queryBuilder.and(FIELD_USERNAME).is(username);
        }
        queryBuilder.and(FIELD_RANDOMIZER).greaterThanEquals(random);
    ...
    {
        DBObject query = BasicDBObjectBuilder.start(FIELD_SUBSCRIBER_ID,
                subscriberId).get();
    ...
    public Stream<SubscriberData> randomSubscribers(int limit)
    {
        ...
        QueryBuilder queryBuilder = QueryBuilder.start(FIELD_STATE)
                .is(DataCreationState.Created.toString()).and(FIELD_RANDOMIZER)
                .greaterThanEquals(random);
        DBObject queryObj = queryBuilder.get();
        ...
        if (limit > 0 && count < limit)
        {
            queryObj.put(FIELD_RANDOMIZER,
                    BasicDBObjectBuilder.start("$lte", random).get());
            count = collection.count(queryObj);
            if (limit > 0 && count < limit)
            {
                throw new RuntimeException("Not enough subscribers for limit "
                        + limit);
            }
        ...
        Stream<SubscriberData> stream = StreamSupport
                .stream(cur.spliterator(), false).onClose(() -> cur.close())
                .map(dbo -> SubscriberData.fromDBObject(dbo)); // need to close
                                                               // cursor;
        return stream;
    }
}
