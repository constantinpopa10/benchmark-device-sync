...
public class CreatePeriodicSubscriptions extends AbstractEventProcessor
{
     ...
     */
    public CreatePeriodicSubscriptions(SubscribersService subscribersService, SubscriptionsService subscriptionsService,
    ...
    protected EventResult processEvent(Event event) throws Exception
    {
    	...
    	int numSubscriptionsPerBatch = preparePeriodicSubscriptionsData.getNumSubscriptionsPerBatch();
    	int timeBetweenBatches = preparePeriodicSubscriptionsData.getTimeBetweenBatches();
        ...
        try
        {
        	for(long i = 0; i < numSubscriptionsPerBatch; i++)
        	{
        		...
        		{
        			path = "/Company Home/Sites/" + siteData.getSiteId() + "/documentLibrary";
        		}
        		Subscription subscription = alfresco.createSubscription("-default-", subscriberData.getSubscriberId(),
        				subscriptionType, path);
            	subscriptionsService.addSubscription(username, subscriberData.getSubscriberId(), subscription.getId(),
            			subscriptionType.toString(), path, DataCreationState.Created);
        	}

        	long scheduledTime = System.currentTimeMillis() + timeBetweenBatches;
        	CreatePeriodicSubscriptionsData newPreparePeriodicSubscriptionsData =
        			new CreatePeriodicSubscriptionsData(numSubscriptionsPerBatch, timeBetweenBatches,
        					max, count + 1);
        	Event nextEvent = new Event("createPeriodicSubscriptions", scheduledTime, newPreparePeriodicSubscriptionsData);
            List<Event> nextEvents = new LinkedList<>();
        	nextEvents.add(nextEvent);
            String msg = "Prepared " + numSubscriptionsPerBatch + " subscriptions and raising '" + event.getName()
                    + "' event.";
